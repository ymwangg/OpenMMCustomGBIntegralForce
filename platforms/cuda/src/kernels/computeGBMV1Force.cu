...
float deltaR_qj = __sqrtf(delta.x*delta.x + delta.y*delta.y + delta.z*delta.z);
...
float gammaj = GAMMA * __logf(LAMBDA) / (atomicRadii_j4);
float factor2 = factor*gammaj*__expf(gammaj*deltaR4)*(4.0*deltaR3);
