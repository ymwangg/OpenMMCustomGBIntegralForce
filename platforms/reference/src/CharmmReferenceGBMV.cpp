...
CharmmReferenceGBMV::CharmmReferenceGBMV(int numberOfAtoms){
...
void CharmmReferenceGBMV::setNeighborList(OpenMM::NeighborList& neighborList){
    _neighborList = &neighborList;
}

OpenMM::NeighborList* CharmmReferenceGBMV::getNeighborList(){
    return _neighborList;
}

void CharmmReferenceGBMV::setAtomicRadii(const vector<double>& atomicRadii) {
    ...
    if (atomicRadii.size() == _atomicRadii.size()) {
        ...
        msg << "CharmmReferenceGBMV: input size for atomic radii does not agree w/ current size: input=";
...
const vector<double>& CharmmReferenceGBMV::getAtomicRadii() const {
    return _atomicRadii;
}

void CharmmReferenceGBMV::setScaledRadiusFactors(const vector<double>& scaledRadiusFactors) {
    ...
    if (scaledRadiusFactors.size() == _scaledRadiusFactors.size()) {
        ...
        std::stringstream msg;
        msg << "CharmmReferenceGBMV: input size for scaled radius factors does not agree w/ current size: input=";
...
const vector<double>& CharmmReferenceGBMV::getScaledRadiusFactors() const {
    return _scaledRadiusFactors;
}

void CharmmReferenceGBMV::setSolventDielectric(double solventDielectric) {
    _solventDielectric = solventDielectric;
}

double CharmmReferenceGBMV::getSolventDielectric() const {
    return _solventDielectric;
}

void CharmmReferenceGBMV::setSoluteDielectric(double soluteDielectric) {
    _soluteDielectric = soluteDielectric;
}

double CharmmReferenceGBMV::getSoluteDielectric() const {
    return _soluteDielectric;
}

void CharmmReferenceGBMV::setUseCutoff(double distance) {

     _cutoff         = true;
     _cutoffDistance = distance;
}

void CharmmReferenceGBMV::setNoCutoff(){
    _cutoff = false;
}

void CharmmReferenceGBMV::setNoPeriodic(){
    _periodic = false;
}

void CharmmReferenceGBMV::setPeriodic(){
    _periodic = true;
}

bool CharmmReferenceGBMV::getUseCutoff() const {
     return _cutoff;
}

double CharmmReferenceGBMV::getCutoffDistance() const {
     return _cutoffDistance;
}

void CharmmReferenceGBMV::setPeriodic(OpenMM::Vec3* vectors) {
...
bool CharmmReferenceGBMV::getPeriodic() {
     return _periodic;
}

const OpenMM::Vec3* CharmmReferenceGBMV::getPeriodicBox() {
     return _periodicBoxVectors;
}

double CharmmReferenceGBMV::computeEnergyForces(const vector<Vec3>& atomCoordinates,
    ...
    for (int i = 0; i < numberOfAtoms; ++i){
        ...
        for (int j = 0; j < neighborMatrix[i].size(); ++j){
            ...
            double D_ij = (r_ij*r_ij) / (par_F*R_i*R_j);
            ...
            //compute dG_dr force
            double dG_dr_part1 = q_i * q_j * (4.0-4.0/par_F*exp(-D_ij));
            ...
            if(atomI==atomJ){
                ...
                double dG_dRi = common_part * (R_j + (r_ij*r_ij)/(par_F*R_i));
                double dG_dRj = common_part * (R_i + (r_ij*r_ij)/(par_F*R_j));
...
void CharmmReferenceGBMV::computeBornRadii(const std::vector<OpenMM::Vec3>& atomCoordinates,const std::vector<double>& partialCharges, std::vector<double>& bornRadii){
    //generate quadrature points
    //323     double alpha0 = -0.180; //-0.180
    //324     double alpha1 = 1.817;
    double par_C0 = 1.0;
    double par_C1 = 2.92;
    double par_D = 0.0;
    double par_S = 1.0;
    _dbornR_dr_vec.resize(_numberOfAtoms,std::vector<OpenMM::Vec3>(_numberOfAtoms));
    vector<double> comp_VolumeQuad(_quad.size());
    for(int atomI=0; atomI<_numberOfAtoms; ++atomI){
        ...
        double comp_Int4 = 0.0;
        double comp_Int5 = 0.0;
        double comp_Int7 = 0.0;
        for(int i=0; i<_quad.size(); ++i){
            ...
            if(radius<vdwR) continue;
            double comp_SoluteVolume = computeVolume(atomCoordinates, rQuad);;
            comp_VolumeQuad[i] = comp_SoluteVolume;
            comp_Int4 += weight * comp_SoluteVolume/(radius*radius);
            double radius3 = pow(radius,3);
            comp_Int5 += weight*comp_SoluteVolume/radius3;
            double radius5 = pow(radius,5);
            comp_Int7 += weight*comp_SoluteVolume/radius5;
        }
        double comp_bornR_part1 = 1.0/eta - comp_Int4;
        double comp_bornR_part2 = sqrt(1.0/(2.0*eta*eta) - comp_Int5);
        double comp_bornR_part3 = pow((1.0/(4.0*eta4) - comp_Int7),1.0/4.0);
        cout<<comp_bornR_part1<<" "<<comp_bornR_part2<<endl;
        bornRadii[atomI] = par_S/(par_C0*comp_bornR_part1);
        //cout<<par_C0*comp_bornR_part1 + par_C1*comp_bornR_part2<<endl;
        ...
        for(int i=0; i<_quad.size(); ++i){
            ...
            if(radius == 0)
                continue;
            double part1 = par_C0/(radius*radius);
            double part2 = (1.0/4.0)*par_C1*pow((1.0/(4.0*eta4)-comp_Int7),-3.0/4.0)/radius5;
            double prefactor = (bornRadii[atomI]*bornRadii[atomI])*weight*(part1+part2);
            compute_dbornR_dr_vec(atomCoordinates, atomI, prefactor, rQuad, comp_VolumeQuad[i]);
        }
    }
    return;
}
...
double CharmmReferenceGBMV::computeVolume(const std::vector<OpenMM::Vec3>& atomCoordinates, const OpenMM::Vec3& quadCoordinate){
    double part1 = 0.0; //
    double gamma0 = 0.44;
    //double lambda = 0.5;
    double lambda = 0.1;
    ...
    for(int atomJ=0; atomJ<_numberOfAtoms; ++atomJ){
        ...
        double dr = deltaR[ReferenceForce::RIndex];
        double RatomJ = _atomicRadii[atomJ];
        double dr4 = pow(dr,4);
        double gammaJ = gamma0*log(lambda)/(RatomJ*RatomJ*RatomJ*RatomJ);
        part1 += exp(gammaJ*dr4);
    } 
    double Volume =  1.0/(1.0+exp(beta*(part1 - lambda)));
    //cout<<beta<<" "<<part1<<" "<<lambda<<endl;
    return Volume;
}

void CharmmReferenceGBMV::compute_dbornR_dr_vec(const std::vector<OpenMM::Vec3>& atomCoordinates, const int atomI, const double prefactor, const OpenMM::Vec3& quadCoordinate, const double volumeI){
...
void CharmmReferenceGBMV::computeLookupTable(const vector<Vec3>& atomCoordinates){
    //_lookupTable;
    //
    //_r1;
...
std::vector<int> CharmmReferenceGBMV::getLookupTableAtomList(OpenMM::Vec3 point){
    vector<int> atomList;
...
void CharmmReferenceGBMV::computeBornRadiiFast(const std::vector<OpenMM::Vec3>& atomCoordinates,const std::vector<double>& partialCharges, std::vector<double>& bornRadii){
    ...
    for(int atomI=0; atomI<_numberOfAtoms; ++atomI){
        ...
        for(int i=0; i<_quad.size(); ++i){
            ...
            integral2 += weight * molecularVolume/radius3;
        }
        double inverseBornRi = alpha0*(1.0/eta - integral1) + 
            //alpha1*pow((1.0/(4.0*eta4) - integral2), 1.0/4.0);
            alpha1*pow((1.0/(2.0*eta*eta) - integral2), 1.0/2.0);
        bornRadii[atomI] = 1.0/inverseBornRi - 0.05;
        cout<<bornRadii[atomI]<<endl;
        //cout<<bornRadii[atomI]<<endl;
...
double CharmmReferenceGBMV::computeVolumeFast(const std::vector<OpenMM::Vec3>& atomCoordinates, const OpenMM::Vec3& quadCoordinate, const double switchDistance, const std::vector<int>& atomList){
    double part1 = 0.0; //
    double gamma0 = 0.44;
    double lambda = 0.1;
    double beta = -100.0;
    ...
    double oneMinusV = 1.0 - 1.0/(1.0+exp(beta*(part1 - lambda)));
    return oneMinusV;
}

void CharmmReferenceGBMV::compute_dbornR_dr_vec_Fast(const std::vector<OpenMM::Vec3>& atomCoordinates, const int atomI, const double prefactor, const OpenMM::Vec3& quadCoordinate, const double volumeI, const double switchDistance, const std::vector<int>& atomList){
    //_dbornR_dr_vec;
    if(volumeI==0) return;
